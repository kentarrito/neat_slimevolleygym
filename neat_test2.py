...
def eval_genomes(genomes, config):
    global obs, manualMode, otherManualAction, otherManualMode

    total_net_action = np.array([0.0,0.0,0.0])
    total_action = np.array([0.0,0.0,0.0])
    best_reward = -1000
    for genome_id, genome in genomes:
        genome.fitness = 0
        net = neat.nn.FeedForwardNetwork.create(genome, config)
